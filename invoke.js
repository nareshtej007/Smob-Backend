...
var options = {
    ...
    peer_url_1: 'grpc://139.59.27.247:8051',
    peer_url_2:'grpc://206.189.138.133:9051',
    event_url: 'grpc://139.59.27.247:7053',
    orderer_url: 'grpc://139.59.27.247:7050'
};
...
function newRequest(params) {
    ...
    return Promise.resolve().then(async () => { 
        ...
        console.log("Set wallet path,and associate user", options.user_id," with application",wallet);
        ...
        console.log("Check user is enrolled, and set a query URL in the network",user);
        if (user === undefined || user.isEnrolled() === false) {
            console.error("User not defined, or not enrolled - error");
        }
        channel = client.newChannel(options.channel_id);
        var peerObj = client.newPeer(options.peer_url_1);
        var peerObj1 = client.newPeer(options.peer_url_2);
        channel.addPeer(peerObj);
        channel.addPeer(peerObj1);
        channel.addOrderer(client.newOrderer(options.orderer_url));
        targets.push(peerObj,peerObj1);
        return;
    }).then(() => {
